...
constructor(
    ...
    suspend fun connect(url: String, token: String, options: ConnectOptions = ConnectOptions()) = coroutineScope {
        ...
        error?.let {
            if (it !is CancellationException) {
                handleDisconnect(reason = DisconnectReason.JOIN_FAILURE)
            }
            throw it
        }
    }
    ...
    fun disconnect() {
        ...
        handleDisconnect(reason = DisconnectReason.CLIENT_INITIATED)
    }
    ...
    override fun onJoinResponse(response: LivekitRtc.JoinResponse) {
        ...
        if (response.otherParticipantsList.isNotEmpty()) {
            response.otherParticipantsList.forEach { info ->
                getOrCreateRemoteParticipant(identity = Participant.Identity(info.identity), info = info)
            }
        }
    }
            ...
            localParticipant.events.collect {
                when (it) {
                    ...
                    is ParticipantEvent.MetadataChanged -> {
                        emitWhenConnected(event = RoomEvent.ParticipantMetadataChanged(
                                this@Room,
                                it.participant,
                                it.prevMetadata,
                            ))
                    ...
                    is ParticipantEvent.AttributesChanged -> {
                        emitWhenConnected(event = RoomEvent.ParticipantAttributesChanged(
                                this@Room,
                                it.participant,
                    ...
                    is ParticipantEvent.NameChanged -> {
                        emitWhenConnected(event = RoomEvent.ParticipantNameChanged(
                                this@Room,
                                it.participant,
                                it.name,
                            ))
    ...
    fun getParticipantBySid(sid: String): Participant? {
        return getParticipantBySid(sid = Participant.Sid(sid))
    }
    ...
    fun getParticipantByIdentity(identity: String): Participant? {
        return getParticipantByIdentity(identity = Participant.Identity(identity))
    }
    ...
    private fun handleActiveSpeakersUpdate(speakerInfos: List<LivekitModels.SpeakerInfo>) {
        ...
        speakerInfos.forEach { speakerInfo ->
            ...
            val participant = getParticipantBySid(sid = speakerSid) ?: return@forEach
    ...
    private fun cleanupRoom() {
        ...
        remoteParticipants.keys.toMutableSet() // copy keys to avoid concurrent modifications.
            .forEach { sid -> handleParticipantDisconnect(identity = sid) }
    ...
    fun sendSimulateScenario(scenario: SimulateScenario) {
        ...
        sendSimulateScenario(scenario = builder.build())
    }
    ...
    override fun onAddTrack(receiver: RtpReceiver, track: MediaStreamTrack, streams: Array<out MediaStream>) {
        ...
        var (participantSid, streamId) = unpackStreamId(packed = streams.first().id)
        var trackSid = track.id()

        if (streamId != null && streamId.startsWith("TR")) {
            trackSid = streamId
        }
        val participant = getParticipantBySid(sid = participantSid) as? RemoteParticipant
    ...
    override fun onUpdateParticipants(updates: List<LivekitModels.ParticipantInfo>) {
        for (info in updates) {
            ...
            if (info.state == LivekitModels.ParticipantInfo.State.DISCONNECTED) {
                handleParticipantDisconnect(identity = participantIdentity)
            } else {
                val participant = getOrCreateRemoteParticipant(identity = participantIdentity, info = info)
                if (isNewParticipant) {
    ...
    override fun onActiveSpeakersUpdate(speakers: List<LivekitModels.SpeakerInfo>) {
        handleActiveSpeakersUpdate(speakerInfos = speakers)
    }
    ...
    override fun onConnectionQuality(updates: List<LivekitRtc.ConnectionQualityInfo>) {
        updates.forEach { info ->
            ...
            val participant = getParticipantBySid(sid = info.participantSid) ?: return@forEach
    ...
    override fun onSpeakersChanged(speakers: List<LivekitModels.SpeakerInfo>) {
        handleSpeakersChanged(speakerInfos = speakers)
    }
    ...
    override fun onUserPacket(packet: LivekitModels.UserPacket, kind: LivekitModels.DataPacket.Kind, encryptionType: LivekitModels.Encryption.Type) {
        val participant = getParticipantBySid(sid = packet.participantSid) as? RemoteParticipant
    ...
    override fun onTranscriptionReceived(transcription: LivekitModels.Transcription) {
        ...
        val participant = getParticipantByIdentity(identity = transcription.transcribedParticipantIdentity)
    ...
    override fun onStreamStateUpdate(streamStates: List<LivekitRtc.StreamStateInfo>) {
        for (streamState in streamStates) {
            val participant = getParticipantBySid(sid = streamState.participantSid) ?: continue
    ...
    override fun onSubscriptionPermissionUpdate(subscriptionPermissionUpdate: LivekitRtc.SubscriptionPermissionUpdate) {
        val participant = getParticipantBySid(sid = subscriptionPermissionUpdate.participantSid) as? RemoteParticipant ?: return
    ...
    override fun onFullReconnecting() {
        ...
        remoteParticipants.keys.toMutableSet() // copy keys to avoid concurrent modifications.
            .forEach { identity -> handleParticipantDisconnect(identity = identity) }
    }
    ...
    override fun onLocalTrackSubscribed(response: LivekitRtc.TrackSubscribed) {
        ...
        coroutineScope.launch {
            emitWhenConnected(event = RoomEvent.LocalTrackSubscribed(this@Room, publication, this@Room.localParticipant))
        }
...
sealed class RoomException(message: String? = null, cause: Throwable? = null) :
    ...
    class ConnectException(message: String? = null, cause: Throwable? = null) :
        RoomException(message = message, cause = cause)
}
