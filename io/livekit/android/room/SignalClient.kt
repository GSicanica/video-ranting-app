...
constructor(
    ...
    suspend fun join(
        ...
        val joinResponse = connect(url = url, token = token, options = options, roomOptions = roomOptions)
    ...
    private suspend fun connect(
        ...
        val wsUrlString = "${url.toWebsocketUrl()}/rtc${createConnectionParams(clientInfo = getClientInfo(), options = options, roomOptions = roomOptions)}"
    ...
    private fun createConnectionParams(
        ...
        addParam(key = CONNECT_QUERY_PROTOCOL, value = options.protocolVersion.value.toString())

        if (options.reconnect) {
            addParam(key = CONNECT_QUERY_RECONNECT, value = "1")
            options.participantSid?.let { sid ->
                addParam(key = CONNECT_QUERY_PARTICIPANT_SID, value = sid)
            }
        }

        val autoSubscribe = if (options.autoSubscribe) 1 else 0
        addParam(key = CONNECT_QUERY_AUTOSUBSCRIBE, value = autoSubscribe.toString())

        val adaptiveStream = if (roomOptions.adaptiveStream) 1 else 0
        addParam(key = CONNECT_QUERY_ADAPTIVE_STREAM, value = adaptiveStream.toString())

        // Client info
        addParam(key = CONNECT_QUERY_SDK, value = "android")
        addParam(key = CONNECT_QUERY_VERSION, value = clientInfo.version)
        addParam(key = CONNECT_QUERY_DEVICE_MODEL, value = clientInfo.deviceModel)
        addParam(key = CONNECT_QUERY_OS, value = clientInfo.os)
        addParam(key = CONNECT_QUERY_OS_VERSION, value = clientInfo.osVersion)
        addParam(key = CONNECT_QUERY_NETWORK_TYPE, value = networkInfo.getNetworkType().protoName)
            ...
            if (responseFlowJob == null) {
                responseFlowJob = coroutineScope.launch {
                    responseFlow.collect { (ws, response) ->
                        ...
                        handleSignalResponseImpl(ws = ws, response = response)
                    }
                }
            }
        }
    ...
    override fun onMessage(webSocket: WebSocket, bytes: ByteString) {
        ...
        handleSignalResponse(ws = webSocket, response = response)
    }
    ...
    override fun onClosed(webSocket: WebSocket, code: Int, reason: String) {
        ...
        handleWebSocketClose(reason = reason, code = code)
    }
    ...
    private fun sendRequest(request: LivekitRtc.SignalRequest) {
        ...
        if (skipQueue) {
            sendRequestImpl(request = request)
        } else {
        ...
        if (pingJob == null && pingIntervalDurationMillis != 0L) {
            pingJob = coroutineScope.launch {
                while (true) {
                    ...
                    startPingTimeout(timestamp = pingTimestamp)
                }
            }
        }
    }
...
//            iceServer(url = "stun:stun2.l.google.com:19302"),
//            iceServer(url = "stun:stun3.l.google.com:19302"),
//            iceServer(url = "stun:stun4.l.google.com:19302"),
        )
