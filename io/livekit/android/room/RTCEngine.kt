...
internal constructor(
    ...
    suspend fun join(
        ...
        return joinImpl(url = url, token = token, options = options, roomOptions = roomOptions)
    }
            ...
            configurationLock.withCheckLock(
                ...
                if (joinResponse.subscriberPrimary) {
                    ...
                    subscriberObserver.dataChannelListener = onDataChannel@{ dataChannel: DataChannel ->
                        ...
                        dataChannel.registerObserver(DataChannelObserver(dataChannel = dataChannel))
                    }
                ...
                reliableDataChannel = publisher?.withPeerConnection {
                    createDataChannel(
                        ...
                        val dataChannelManager = DataChannelManager(dataChannel, DataChannelObserver(dataChannel = dataChannel), rtcThreadToken)
                ...
                lossyDataChannel = publisher?.withPeerConnection {
                    createDataChannel(
                        ...
                        lossyDataChannelManager = DataChannelManager(dataChannel, DataChannelObserver(dataChannel = dataChannel), rtcThreadToken)
        ...
        val job = coroutineScope.launch {
            ...
            for (retries in 0 until MAX_RECONNECT_RETRIES) {
                ...
                if (isFullReconnect) {
                    ...
                    try {
                        closeResources(reason = "Full Reconnecting")
                        listener?.onFullReconnecting()
                        joinImpl(url = url!!, token = token, options = connectOptions, roomOptions = lastRoomOptions ?: RoomOptions())
                    } catch (e: Exception) {
                ...
                if (connectionState == ConnectionState.CONNECTED &&
                    ...
                    if (lastMessageSeq != null) {
                        resendReliableMessagesForResume(lastMessageSeq = lastMessageSeq)
                    }
            ...
            close(reason = "Failed reconnecting")
    ...
    internal suspend fun sendData(dataPacket: LivekitModels.DataPacket): Result<Unit> {
        ensurePublisherConnected(kind = dataPacket.kind)
        ...
        fun sendDataImpl(dataPacket: LivekitModels.DataPacket): Result<Unit> {
            try {
                ...
                val channel = dataChannelForKind(kind = dataPacket.kind)
        ...
        if (dataPacket.kind == LivekitModels.DataPacket.Kind.RELIABLE) {
            synchronized(reliableStateLock) {
                return sendDataImpl(dataPacket = dataPacket)
            }
        } else {
            return sendDataImpl(dataPacket = dataPacket)
        }
    ...
    internal suspend fun resendReliableMessagesForResume(lastMessageSeq: Int): Result<Unit> {
        ensurePublisherConnected(kind = LivekitModels.DataPacket.Kind.RELIABLE)
        val channel = dataChannelForKind(kind = LivekitModels.DataPacket.Kind.RELIABLE)
    ...
    internal suspend fun waitForBufferStatusLow(kind: LivekitModels.DataPacket.Kind) {
        try {
            ensurePublisherConnected(kind = kind)
        } catch (e: Exception) {
            return
        }
    ...
    private suspend fun ensurePublisherConnected(kind: LivekitModels.DataPacket.Kind) {
        ...
        val targetChannel = dataChannelForKind(kind = kind) ?: throw RoomException.ConnectException("Publisher isn't setup yet! Is room not connected?!")
    ...
    fun sendSyncState(
        ...
        val dataChannelInfos = LivekitModels.DataPacket.Kind.entries
            ...
            .mapNotNull { kind -> dataChannelForKind(kind = kind) }
            .map { dataChannel ->
