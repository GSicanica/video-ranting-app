...
fun VideoTrackView(
    ...
    fun updateView(v: View) {
        ...
        setupVideoIfNeeded(videoTrack = videoTrack, view = compat.sink)
                ...
                factory = { context ->
                    FrameLayout(context).apply {
                        val renderer = SurfaceViewRenderer(context).apply {
                            ...
                            setupVideoIfNeeded(videoTrack = videoTrack, view = this)
                        }
                        addView(
                            renderer,
                        ...
                        val compat = SurfaceRendererCompat(view = renderer)
                        tag = compat
            ...
            AndroidView(
                factory = { context ->
                    FrameLayout(context).apply {
                        ...
                        val compat = TextureRendererCompat(view = renderer)
                        tag = compat
                        rendererCompat = compat
                    }
                },
                update = { v -> updateView(v = v) },
                modifier = modifier
                    .onGloballyPositioned { videoSinkVisibility.onGloballyPositioned(it) },
            )
        }
